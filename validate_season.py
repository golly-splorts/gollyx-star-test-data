...
for iseason in range(LAST_SEASON0 + 1):
    ...
    scslen = len(bracket["SCS"])
    if scslen != 7:
        raise Exception(
            f"Error: postseason SCS length is invalid: {scslen} games, should be 7"
        )
    ...
    scslen = len(postseason["SCS"])
    if scslen > 7 or scslen < 4:
        raise Exception(f"Error: postseason HCS length is invalid: {scslen} games")
